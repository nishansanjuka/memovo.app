...
$env:GATEWAY_POSTMAN_ENVIRONMENT_ID = $env:GATEWAY_POSTMAN_ENVIRONMENT_ID
...
$env_data = Get-Content "$env:TEMP\gateway-postman-environment.json" -Raw
$body = @{ environment = (ConvertFrom-Json $env_data) } | ConvertTo-Json -Depth 100

try {
    # Try to get the environment to check if it exists
    Invoke-RestMethod -Uri "https://api.getpostman.com/environments/$env:GATEWAY_POSTMAN_ENVIRONMENT_ID" -Method GET -Headers @{ 'X-Api-Key' = $env:POSTMAN_API_KEY } -ErrorAction Stop | Out-Null
    
    Write-Host "Environment exists, updating..."
    Invoke-RestMethod -Uri "https://api.getpostman.com/environments/$env:GATEWAY_POSTMAN_ENVIRONMENT_ID" -Method PUT -Headers @{ 'X-Api-Key' = $env:POSTMAN_API_KEY; 'Content-Type' = 'application/json' } -Body $body -ErrorAction Stop | Out-Null
...
Write-Host "Done! Gateway environment synced to Postman!"
